...
void deleteEntry(FILE *file, struct Stud *students, int *size)
{
    ...
    for (int i = 0; i < *size && don_t == 0; i++)
    {
        ...
        {
            fprintf(file, "%s %s %d %d %s %d %s %d %d", students[i].name, students[i].sex, students[i].age, 
                                                        students[i].avg_exam_pts, students[i].enroll_date, students[i].flag, 
                                                        students[i].Home_t.Flat.street, students[i].Home_t.Flat.house_num, students[i].Home_t.Flat.flat_num);    
        }
        else
        {
            fprintf(file, "%s %s %d %d %s %d %d %d", students[i].name, students[i].sex, students[i].age, 
                                                     students[i].avg_exam_pts, students[i].enroll_date, students[i].flag, 
                                                     students[i].Home_t.Hostel.hostel_num, students[i].Home_t.Hostel.flat_num);
        }
    ...
    if (*error == ERROR_NONE)
    {
        ...
        if (collector == 6)
        {
            if (adder.flag == 1)
            {
                ...
                collector += scanf("%s", adder.Home_t.Flat.street);
                collector += scanf("%d", &adder.Home_t.Flat.house_num);
                collector += scanf("%d", &adder.Home_t.Flat.flat_num);
            ...
            else
            {
                ...
                collector += scanf("%d", &adder.Home_t.Hostel.hostel_num);
                collector += scanf("%d", &adder.Home_t.Hostel.flat_num);
        ...
        if (*error == ERROR_NONE)
        {
            if (adder.flag == 1)
            {
                printf("\n");
                printStruct(adder);
                fprintf(file, "\n%s %s %d %d %s %d %s %d %d", adder.name, adder.sex, adder.age, adder.avg_exam_pts, 
                                                                adder.enroll_date, adder.flag, adder.Home_t.Flat.street, 
                                                                    adder.Home_t.Flat.house_num, adder.Home_t.Flat.flat_num);
            ...
            else
            {
                printf("\n");
                printStruct(adder);
                fprintf(file, "\n%s %s %d %d %s %d %d %d", adder.name, adder.sex, adder.age, adder.avg_exam_pts, 
                                                            adder.enroll_date, adder.flag, adder.Home_t.Hostel.hostel_num, 
                                                                adder.Home_t.Hostel.flat_num);
...
void printStruct(struct Stud students)
{
    ...
    if (students.flag == 1)
    {
        printf(" %20s |", students.Home_t.Flat.street);
        printf(" %18d |", students.Home_t.Flat.house_num);
        printf(" %15d |\n", students.Home_t.Flat.flat_num);
    }
    else
    {
        printf(" %20s |", "----------");
        printf(" %18d |", students.Home_t.Hostel.hostel_num);
        printf(" %15d |\n", students.Home_t.Hostel.flat_num);
    }
}
...
int readFile(FILE *file, struct Stud *students, int *error)
{
    ...
    for (int i = 0; !feof(file); i++, amount++, counter++)
    {
        if (fscanf(file, "%s %s %d %d %s %d", students[i].name, students[i].sex, &students[i].age, &students[i].avg_exam_pts, students[i].enroll_date, &students[i].flag) == 6)
        {
            if (students[i].flag == 1)
            {
                if (fscanf(file, "%s %d %d", students[i].Home_t.Flat.street, &students[i].Home_t.Flat.house_num, &students[i].Home_t.Flat.flat_num) != 3)
                {
                ...
                if (fscanf(file, "%d %d", &students[i].Home_t.Hostel.hostel_num, &students[i].Home_t.Hostel.flat_num) != 2)
                {
                    printf("Error: File reading error\n");
                    *error = ERROR_FILE;
                    break;   
